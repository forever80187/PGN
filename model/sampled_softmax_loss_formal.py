...
class SampledSoftmaxLoss(torch.nn.Module):
    ...
    def forward(self,
                embeddings: Variable,
                targets: Variable,
                target_token_embedding: Variable = None) -> Variable:
        ...
        if embeddings.shape[0] == 0:
            # empty batch
            return Variable(torch.tensor(0.0).cpu())  # pylint: disable=not-callable
    ...
    def _forward_train(self,
                       embeddings: Variable,
                       targets: Variable,
                       target_token_embedding: Variable) -> Variable:
        ...
        long_targets.requires_grad = False
        # long_targets = long_targets.cuda()
        # sampled_ids = sampled_ids.cuda()
        ...
        # _, tag_seq = torch.max(log_softmax, 1)
        nll_loss = -1.0 * log_softmax[:, 0].sum()
        return nll_loss

    def _forward_eval(self, embeddings: Variable, targets: Variable) -> Variable:
    ...
    def log_uniform_candidate_sampler(self, targets, choice_func=_choice):
        ...
        sampled_ids = Variable(torch.from_numpy(np_sampled_ids).cpu())
        ...
        sampled_ids.requires_grad = False
        target_expected_count.requires_grad = False
        sampled_expected_count.requires_grad = False

        if self.gpu:
            sampled_ids = sampled_ids.cuda()
            target_expected_count = target_expected_count.cuda()
            sampled_expected_count = sampled_expected_count.cuda()
