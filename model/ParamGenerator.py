    ...
    def forward(self, task_emb, domain_emb):
        ...
        for idx_layer in range(self.layer_num):
            for direct in range(self.direct_num):
                ...
                w_ih = param_matrix[0, fund_position: fund_position + gate_size * layer_input_size].view(gate_size, layer_input_size)
                fund_position += gate_size * layer_input_size

                w_hh = param_matrix[0, fund_position: fund_position + gate_size * self.hidden_size].view(gate_size, self.hidden_size)
...
class LSTMParamGenerator(Module):
    ...
    def __init__(self, input_size, hidden_size, data):
        ...
        self.cpg_ = ParamGenerator(input_size, hidden_size, task_emb_size, domain_emb_size, 
                                   layer_num, bilstm_flag, bias=True)
